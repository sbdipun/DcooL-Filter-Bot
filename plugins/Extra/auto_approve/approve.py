...
async def auto_approve(client, message: ChatJoinRequest):
    if message.chat.id == get_random_channel_id() and join_db().isActive():
    ...
    if AUTO_APPROVE_MODE == True:
        ...
        if message.chat.id == get_random_channel_id():

            return 
        chat = message.chat 
        user = message.from_user  
        await client.approve_chat_join_request(chat_id=chat.id, user_id=user.id)
        text = f"<b>ʜᴇʟʟᴏ {message.from_user.mention} 👋,\n\nYᴏᴜʀ Rᴇǫᴜᴇsᴛ ᴛᴏ Jᴏɪɴ {message.chat.title} ɪs Aᴘᴘʀᴏᴠᴇᴅ.</b>"
