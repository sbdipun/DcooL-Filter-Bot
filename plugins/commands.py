...
async def start(client, message):
    ...
    if get_random_channel_id() and not await is_subscribed(client, message):
        try:
            if REQUEST_TO_JOIN_MODE == True:
                invite_link = await client.create_chat_invite_link(chat_id=(int(get_random_channel_id())), creates_join_request=True)
            else:
                invite_link = await client.create_chat_invite_link(int(get_random_channel_id()))
